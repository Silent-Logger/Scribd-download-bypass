...
// @version      2.3.0
// @description  📚 Download documents from Scribd as PDF (fork with tweaks)
// @author       ThanhNguyxn
...
(function () {
  ...
  const PAGE_LOAD_TIMEOUT_MS = 4000;
  ...
  function getPageElements() {
    let pages = qa('.page, .outer_page, .new_page, [data-page-number]');
    if (pages.length === 0) {
      pages = qa("[class*='page']");
    }
    return pages.filter(node => node.offsetHeight > 200 && node.offsetWidth > 200);
  }
  ...
  function isPageLoaded(page) {
    try {
      ...
      const hasImg = page.querySelector('img, svg, canvas');
      return !!hasImg;
    } catch (e) {
      return false;
    }
  ...
  async function startDownload() {
    ...
    try {
      ...
      if (loadedCount < totalPages * 0.8) {
        setProgress(100, '⚠️ Some pages may be incomplete. Check the print preview.');
        await sleep(1500);
      }
